...
def parse_verilog_state_machine(verilog_code):
    ...
    transition_regex = re.compile(r'if\s*\((.*?)\)\s*(begin\s*)?(.*?)\s*(?=else|elsif|end|\w+:)', re.DOTALL)
...
def create_state_machine_diagram(states, transitions, output_file='state_machine'):
    ...
    fsm.attr(rankdir='TB', splines='true', nodesep='0.5', ranksep='0.7')  # Normal splines with better layout for labels
    ...
    for state in ordered_states:
        if state in states:
            if state == 'RXU_RESET_IDLE':  # Mark initial state with bold outline
                fsm.node(state, style='bold')  # Bold outline for the initial state
            else:
                fsm.node(state)
